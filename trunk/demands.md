...
2.  时间描述字数限制逻辑优化（中文字符按照英文字符长度的1.5倍计算，计英文字符一个为长度1，总共长度不应超过25）
3.  新增功能：note功能，如果一个事件很长，标题放不下，可以放在note里。note是一个按钮，在delete和update后面，点击后会显示一个textbox
4.  刷新逻辑优化
5.  bug fix：事件通过recurring添加后，点击删除后的一系列bug
